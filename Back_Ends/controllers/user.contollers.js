...
export const getUsers = async (req, res) => {
  try {
    const getUserData = await userModels.find({ status: 1 });
    if (getUserData) {
...
export const getUser = async (req, res) => {n
  try {
    const userID = req.params.user_id;
    const getUserData = await userModels.findOne({ _id: userID, status: 1 });
    if (getUserData) {
      return res.status(200).json({
        data: getUserData,
...
export const updateUser = async (req, res) => {
  try {
    ...
    const updated = await userModels.updateOne(
      { _id: userID },
      {
        $set: {
          firstName: firstName,
...
export const deleteUser = async (req, res) => {
  try {
    ...
    const deleted = await userModels.deleteOne({ _id: userID });
...
export const softDeleteUser = async (req, res) => {
  try {
    ...
    const deleted = await userModels.updateOne(
      { _id: userID },
      { $set: { status: 0 } }
    );
    if (deleted.acknowledged) {
